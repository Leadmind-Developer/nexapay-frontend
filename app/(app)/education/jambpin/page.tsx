...
/* ================= PAGE ================= */
export default function JambPinPage() {
  const serviceID = "jamb";
  const verifyEndpoint = "/vtpass/education/jamb/verify"; // updated
  ...
  const verifyCandidate = async () => {
    ...
    try {
      ...
      const res = await api.post(verifyEndpoint, {
        serviceID,
        billersCode,
      ...
      setStage("error");
      setErrorMessage(
        err?.response?.data?.error || "Unable to verify JAMB profile"
      );
    }
  };
      ...
      <div className="max-w-md mx-auto px-4 py-8 space-y-4 text-gray-900 dark:text-gray-100">
        ...
        {stage === "form" && (
          <div className="bg-white dark:bg-gray-900 border dark:border-gray-800 rounded-lg p-6 space-y-4 shadow">
            <h2 className="text-xl font-bold">JAMB PIN (UTME / Direct Entry)</h2>
        ...
        {stage === "success" && (
          <div className="bg-green-100 dark:bg-green-900 border p-6 rounded text-center space-y-3">
            ...
            <p className="text-sm opacity-80">
              Your JAMB PIN will appear in Transactions shortly.
            </p>
          </div>
        )}
