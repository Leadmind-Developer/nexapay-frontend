...
export default function ElectricityPage() {
  ...
  const handleCheckout = async () => {
    ...
    try {
      ...
      const vtpassTxStatus =
        vtpass?.transactions?.status?.toUpperCase() ||
        res.data?.status?.toUpperCase() ||
        "PROCESSING";

      const newReceipt: Receipt & { vtpass?: any } = {
        requestId: res.data.requestId,
        meter_number: verification.meter_number,
        type,
        customer_name: verification.customer_name,
        amount: Number(amount),
        token: vtpass?.token || vtpass?.token_code || vtpass?.purchased_code || null,
        status: vtpassTxStatus,
        vtpass,
      };
      ...
      setStage(
        vtpassTxStatus === "FAILED"
          ? "error"
          : vtpassTxStatus === "SUCCESS" || vtpassTxStatus === "DELIVERED"
          ? "success"
          : "processing"
      );
    } catch (err: any) {
    ...
    <BannersWrapper page="electricity">
      <div className="max-w-md mx-auto px-4 space-y-4 text-gray-900 dark:text-gray-100">
          ...
          <div className="bg-white dark:bg-gray-900 border dark:border-gray-800 rounded-lg p-5 shadow space-y-4">
            ...
            <div className="space-y-3">
              <input type="number" value={amount} onChange={e => setAmount(e.target.value)}
                placeholder="Amount" className="w-full p-3 border rounded dark:bg-gray-900 dark:border-gray-700" />
              <input value={phone} onChange={e => setPhone(e.target.value)}
                placeholder="Phone Number" className="w-full p-3 border rounded dark:bg-gray-900 dark:border-gray-700" />
              {message && <p className="text-red-600 text-sm">{message}</p>}
            </div>
            <div className="flex gap-3">
              <button onClick={() => setStage("verify")}
                className="flex-1 bg-gray-200 dark:bg-gray-700 py-3 rounded">Back</button>
              <button onClick={handleCheckout}
                className="flex-1 bg-yellow-500 text-white py-3 rounded font-semibold">Pay</button>
            </div>
          </div>
        )}
        ...
        {stage === "success" && receipt && (
          <div className="bg-green-100 dark:bg-green-900 border dark:border-green-800 p-6 rounded shadow space-y-4">
            <h2 className="text-lg font-bold">
              {receipt.status === "SUCCESS" ? "Purchase Successful ⚡" : "Purchase Processing ⚡"}
            </h2>
            ...
            <table className="w-full text-left border border-gray-300 dark:border-gray-700 rounded">
              <tbody>
                <tr className="border-b border-gray-300 dark:border-gray-700">
                  ...
                  <td className="p-2">{receipt.token || "Processing…"}</td>
                </tr>
                <tr className="border-b border-gray-300 dark:border-gray-700">
                  <td className="p-2 font-medium">VTpass Ref</td>
                  <td className="p-2">{receipt.vtpass?.exchangeReference || "N/A"}</td>
                </tr>
                <tr>
                  <td className="p-2 font-medium">Units</td>
                  <td className="p-2">{receipt.vtpass?.units || "N/A"}</td>
                </tr>
              </tbody>
            </table>
